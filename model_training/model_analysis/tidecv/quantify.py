...
log = logger.setup_logger(file_name="mode_analysis.log")
...
class TIDEExample:
    ...
    def _run(self):
        ...
        # Some matrices used just for errors calculation
        if self.run_errors:
...
class TIDERun:
    ...
    def _run(self):
        ...
        for image in self.gt.images:
            ...
            # However, since ignored detections are still used for errors calculations, we have to keep them.
            ...
            self._eval_image(x, y, self.gt.images.get(image)['name'])
    ...
    def _eval_image(self, preds: list, gt: list, name: str):
        ...
        for pred_idx, pred in enumerate(preds):
            ...
            if self.run_errors and (pred['used'] == False or pred['used'] == None):
                ...
                if len(ex.gt) == 0:  # Note this is ex.gt because it doesn't include ignore annotations
                    ...
                    log.info(f"BACKGROUND ERROR: {name}")
                    continue
                ...
                if ex.gt_used_cls[pred_idx, idx] >= self.pos_thresh:
                    ...
                    log.info(f"DUPLICATE ERROR: {name}")
                    continue
        ...
        for truth in gt:
            ...
            if not truth['ignore'] and not truth['used']:
                ...
                if self.run_errors:
                    ...
                    # The GT was completely missed, no errors can correct it
                    # Note: 'usable' is set in errors.py
                    if not truth['usable']:
                        self._add_error(MissedError(truth))
                        log.info(f"MISSED ERROR: {name}")
    ...
    def fix_errors(self, condition=lambda x: False, transform=None, false_neg_dict: dict = None,
        ...
        # Potentially fix every errors case
        for error in self.errors:
            if error.disabled:
                continue
    ...
    def fix_main_errors(self, progressive: bool = False, error_types: list = None, qual: Qualifier = None) -> dict:
        ...
        for error in error_types:
            ...
            # If an errors is negative that means it's likely due to binning differences, so just
...
class TIDE:
   ...
   """
    ...
    # This is just here to define a consistent order of the errors types
