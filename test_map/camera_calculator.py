...
The code has been repurposed for Team Livestalk's Fall 2021, W251 final project.
...
class CameraCalculator:
    ...
    @staticmethod
    def getBoundingPolygon(FOVh, FOVv, altitude, roll, pitch, heading, cam_long, cam_lat):
        ...
        Parameters:
            ...
            heading (float): Heading of the camera (z axis) in radians (AKA "yaw")
            roll (float): Roll of the camera (x axis) in radians
            pitch (float): Pitch of the camera (y axis) in radians
            cam_long (float): TEAM LIVESTALK NEW PARAMETER Camera's UTM Easting (x) Position
            cam_lat (float): TEAM LIVESTALK NEW PARAMETER Camera's UTM Northing (y) Position
        Returns:
        ...
        origin = Vector(cam_long, cam_lat, altitude) # cam_long and #cam_lat were previously 0s
